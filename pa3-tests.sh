...
echo -e "\nTesting :: truncate -s 256K BIMDC/test.bin; ./client -w 100 -b 50 -m 4096 -f test.bin; diff -sqwB BIMDC/test.bin received/test.bin\n"
truncate -s 256K BIMDC/test.bin
./client -w 100 -b 50 -m 4096 -f test.bin >/dev/null 2>&1
if test -f "received/test.bin"; then
    if diff BIMDC/test.bin received/test.bin >/dev/null; then
